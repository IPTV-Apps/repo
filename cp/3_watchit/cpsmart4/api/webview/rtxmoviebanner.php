...
<head>
    ...
    <title>RTXMODS: Movie Banner</title>
    <style>
        body {
            margin: 0;
            padding: 0;
        ...
        #movie-poster {
            ...
            -webkit-mask-image: -webkit-gradient(linear,
                    ...
                    from(rgba(0, 0, 0, 0)),
                    to(rgba(0, 0, 0, 0.9)));
            mask-image: linear-gradient(to left, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.9));
        }
...
<body>
    ...
    <script>
        ...
        async function fetchPopularMovieIds_old() {
            ...
            try {
                const response = await fetch(
                    `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&primary_release_date.gte=${lastWeekDateString}&primary_release_date.lte=${currentDateString}&sort_by=popularity.desc`
                );
        ...
        async function fetchPopularMovieIds_old2() {
            ...
            const currentDateString = currentDate.toISOString().split("T")[0];
            const threeMonthsAgoDateString = threeMonthsAgo
                .toISOString()
                .split("T")[0];

            try {
                const response = await fetch(
                    `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&vote_average.gte=7&primary_release_date.gte=${threeMonthsAgoDateString}&primary_release_date.lte=${currentDateString}&sort_by=vote_average.desc`
                );
                const data = await response.json();
                movieIds = data.results.map((movie) => movie.id);
            } catch (error) {
        ...
        async function fetchPopularMovieIds() {
            while (currentPage <= totalPageCount) {
                try {
                    const response = await fetch(
                        `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&page=${currentPage}&sort_by=popularity.desc`
                    );
                    const data = await response.json();
        ...
        function preloadNextImage() {
            ...
            fetch(
                    `https://api.themoviedb.org/3/movie/${nextMovieId}?api_key=${apiKey}`
                )
                ...
                .then((data) => {
                    ...
                    setTimeout(() => {
                        ...
                        const genresArray = data.genres
                            ...
                            .join(" ");
                        const origin_country = data.production_companies
                            .map(
    ...
    <script>
        customElements.define(
            ...
            class RatingBar extends HTMLElement {
                ...
                render() {
                    this.shadowRoot.innerHTML = `<style> span { font-size: 30px; color: white; } .off { filter: grayscale(1); } span:hover { filter: hue-rotate(160deg); cursor: pointer; } </style><div class="rating"></div>`;
